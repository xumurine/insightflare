...
} from "@/lib/demo/analytics/performance";
import {
  DEMO_SITE_PROFILES,
...
} from "@/lib/demo/realtime/browser-client-breakdowns";
import { generateDemoClientCrossBreakdown } from "@/lib/demo/realtime/client-cross";
import type * as FactBuilder from "@/lib/demo/realtime/fact-builder";
import { generateDemoFilterOptions } from "@/lib/demo/realtime/filter-options";
import {
...
vi.mock("@/lib/demo/realtime/fact-builder", async () => {
  const actual = await vi.importActual<typeof FactBuilder>(
    "@/lib/demo/realtime/fact-builder",
  );
  return {
    ...actual,
...
describe("mock remaining generator coverage", () => {
  ...
  it("builds UTM dimension rows, invalid trend fallbacks, and Other trend series", () => {
    ...
    expect((dimension.data as { items: unknown[] }).items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
  ...
  it("normalizes single-segment UTM source labels from fallback entries", () => {
    ...
    expect(
      (result.data as { items: Array<{ value: string }> }).items.some(
  ...
  it("filters direct and empty UTM source labels and falls back for odd source hosts", () => {
    ...
    try {
      ...
      const values = (
        result.data as { items: Array<{ value: string }> }
      ).items.map((row) => row.value);
