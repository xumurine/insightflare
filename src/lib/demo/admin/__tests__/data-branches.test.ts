...
import type * as DemoAdminDurableObjectsModule from "@/lib/demo/admin/durable-objects";
import type * as DemoAdminNotificationsModule from "@/lib/demo/admin/notifications";
import type * as DemoAdminScheduledTasksModule from "@/lib/demo/admin/scheduled-tasks";
import type * as DemoAdminSystemPerformanceModule from "@/lib/demo/admin/system-performance";
import type * as DemoAdminUsersModule from "@/lib/demo/admin/users";
import type { DemoSiteProfile } from "@/lib/demo/data/site-profiles";
import type * as DemoUtilsModule from "@/lib/demo/generators/utils";
type AdminModule = typeof DemoAdminUsersModule &
  typeof DemoAdminNotificationsModule &
  typeof DemoAdminScheduledTasksModule &
  typeof DemoAdminSystemPerformanceModule &
  typeof DemoAdminDurableObjectsModule;
import type {
  DoDiagnosticPayload,
...
async function withMockedAdmin<T>({
  ...
  rngValues?: number[];
  run: (admin: AdminModule) => T | Promise<T>;
}): Promise<T> {
  vi.resetModules();
  vi.doMock("@/lib/demo/data/site-profiles", () => ({
    DEMO_TEAMS: [TEAM],
    DEMO_SITE_PROFILES: siteProfilesExport,
  }));
  vi.doMock("@/lib/demo/realtime/site-curves", () => ({
    integrateViews,
  }));
  vi.doMock("@/lib/demo/generators/utils", async () => {
    const actual = await vi.importActual<typeof DemoUtilsModule>(
      "@/lib/demo/generators/utils",
    );
    return {
      ...actual,
  ...
  try {
    const admin = {
      ...(await import("@/lib/demo/admin/users")),
      ...(await import("@/lib/demo/admin/notifications")),
      ...(await import("@/lib/demo/admin/scheduled-tasks")),
      ...(await import("@/lib/demo/admin/system-performance")),
      ...(await import("@/lib/demo/admin/durable-objects")),
    };
    return await run(admin);
  } finally {
    vi.doUnmock("@/lib/demo/data/site-profiles");
    vi.doUnmock("@/lib/demo/realtime/site-curves");
    vi.doUnmock("@/lib/demo/generators/utils");
    vi.resetModules();
  }
}
    ...
    await withMockedAdmin({
      run: (admin) => {
        ...
        const secondPage = admin.generateDemoScheduledTasks({
          limit: 3,
          cursor: firstPage.runs.pagination.nextCursor ?? "",
          runId: firstPage.runs.items[0]?.id ?? "",
        });

        expect(firstPage.runs.items).toHaveLength(3);
        expect(firstPage.runs.pagination).toEqual({
          limit: 3,
          returned: 3,
          hasMore: true,
          nextCursor: expect.any(String),
        });
        expect(secondPage.runs.items.map((run) => run.id)).not.toContain(
          firstPage.runs.items[0]?.id,
        );
        expect(secondPage.selectedRun?.id).toBe(firstPage.runs.items[0]?.id);
        expect(secondPage.logs.items.length).toBeGreaterThan(0);
      },
    });
  });
