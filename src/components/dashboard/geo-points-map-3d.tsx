...
const DeckOverlay = memo(function DeckOverlay(props: MapboxOverlayProps) {
  ...
  const globeView = useMemo(() => new _GlobeView({ id: "mapbox" }), []);
  const overlayProps = useMemo(
    () => ({ ...props, views: [globeView] }) as unknown as MapboxOverlayProps,
    [globeView, props],
  );
...
export function GeoPointsMap3D({
  locale,
  messages,
  points,
  countryCounts = [],
